...
### FR-2: Implemented as a marked.js inline extension (shared module)
- **Given** the player's `renderTextBlock` and the authoring tool's text-block live preview both render markdown via `marked.parse`
- **When** either module loads
- **Then** both call `registerMarkedExtensions()` from `src/util/markedExtensions.ts`, which registers an inline extension named `dot` matching the literal pattern `{dot}` and emitting the span above. The shared module guarantees the player and the authoring preview render the shortcode identically. Registration is idempotent (safe to call from both consumers).
...
- **Out of scope:** passed markers continue to use their existing hardcoded shade.

### FR-6: Authoring-tool affordance
- **Given** the authoring tool's text-block edit modal
